...
In 1961 Regge developed his calculus replacing smooth differentiable manifolds with simplicial manifolds, which obey the following two properties\\[-4ex]
\begin{enumerate}\addtolength{\itemsep}{-1.5ex}
...
with a circumspherical property of member simplices which may be seen intuitively in \Cref{DT}.
...
the 3D action (Equation 35 from \cite{j._ambjorn_dynamically_2001}):
...
triangulations for computing the path integral. We can then perform measures on these representative ensembles to calculate properties such as spectral dimension.
...
\begin{itemize}\addtolength{\itemsep}{-1.5ex}
  ...
  But from the perspective of computing \Cref{cdt-eq}, a (2,3) move is the mutual inverse of a (3,2) move, because they increase and decrease by one the count of simplices
  and timelike edges. The same applies to the (2,6) and (6,2) moves. The (4,4) move is self-inverse and does essentially nothing, but is still distinct from
  ...
  which makes the depiction of the $\epsilon$-machine intuitively as the bi-infinite line segment.
\section{Methods}
