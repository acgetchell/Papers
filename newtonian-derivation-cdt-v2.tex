...
Thus, the Gauss-Bonnet theorem gives us:

\begin{equation}
K_{top}=K_{bottom}=2\pi\left(1-e^{-\nu (0,z)}\right)
\end{equation}

The "edge" of the top disk contributes nothing to $K$, since as $R\rightarrow\infty$ this surface approaches that of a plane, for which $K=0$.

The Gaussian curvature of the cylinder wall can also be calculated by letting $R\rightarrow\infty$. For this case, the metric of the top and bottom are Minkowski spaces, thus flat planes ($K=0$). Then the entire cylinder is homeomorphic to the 2-sphere, for which the Euler characteristic $\chi=2$ and thus by the Gauss-Bonnet theorem $K=4\pi$. Since the other components (disks and disk edges) have $K=0$ the cylnder wall must contain all of the curvature, that is, $K_{cylinder wall}=K=4\pi$.

Thus,

\begin{equation}
\int KdA=K_{top}+K_{bottom}+K_{cylinder wall}=4\pi\left(1-e^{-\nu (r,z)}\right)+4\pi=2\pi*(2)
\end{equation}
...
Where the delta-function $\delta(r)$ is given by:

\begin{equation}
\delta(r)=\int^{\infty}_{0} \int^{2\pi}_{0} \delta (r)e^{2\left(\lambda-\nu\right)}rdrd\phi =1
\end{equation} 

The two-dimensional sub-manifold in $\left(r,\phi\right)$ is diagonal, 
Thus:
