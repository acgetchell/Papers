...
\Cref{eq:laplace} is the two-dimensional Laplace equation in cylindrical coordinates. Thus, we have:
...
So, our solutions must satisfy \Cref{eq:laplace-r-z,eq:nu}. A particular solution corresponding to two objects is:

\begin{equation}
\lambda_0=-\frac{\mu'}{r'}-\frac{\mu''}{r''}
\label{eq:lambda-0}
\end{equation}

\begin{equation}
\nu_0=\frac{1}{2}\frac{\mu'^{2}r^2}{r'^{4}}-\frac{1}{2}\frac{\mu''^{2}r^2}{r''^{4}}+\frac{2\mu'\mu''}{(z-z'')^2}\left[\frac{r^2+(z-z')(z-z'')}{r'r''}-1\right]
\end{equation}
Where $z'$ and $z''$ correspond to the positions on the z-axis for the two objects, $\mu'$ and $\mu''$ are length parameters, and:
...
\begin{equation}
  \label{eq:lim-2-pi}
  \sin 2\pi\chi + \cos 2\pi\chi = 1\leftarrow WRONG!
\end{equation}
...
Which means that in order for Equation  to hold, our strut must have:
...
Taking the limit as $R\rightarrow\infty$ and recalling elementary flatness (Equation , the Gauss-Bonnet theorem gives:
