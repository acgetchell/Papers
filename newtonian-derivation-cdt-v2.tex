...
These solutions, however, only apply to empty space (recall the use of \Cref{eq:vacuum-solutions}). In addition, as noted by Synge \cite{synge_relativity}, the z axis between $z_1$ and $z_2$ must also be excluded due to violation of elementary flatness. We will examine this in the next section.
...
$r_0\rightarrow 0$ is, from \Cref{eq:v-from-par-transport}:
...
$\chi=n=1,2,\ldots$ and $n=\frac{1}{4}, \frac{9}{4}, \frac{17}{4}$. The equivalent to \Cref{eq:lim-0,eq:lim-2-pi} for
arbitrary $r_0$ from \Cref{eq:v-from-par-transport} are much more
complex.
...
<<<<<<< HEAD
Outside the strut, we should get the vacuum solutions, which implies $f(r,z)=g(r,z)$.
=======
In empty space outside the strut \Cref{eq:weyl-metric} reduces to \Cref{eq:weyl-vacuum-metric} which implies $f(r,z)=g(r,z)$.

Recall from \Cref{eq:nu-elem-flat}
>>>>>>> Editing \S1.3
...
Continuing on, and using \Cref{eq:nu-elem-flat,eq:h-limit} we obtain:
...
Which means that in order for \Cref{}  to hold, our strut must have:
...
Recall that earlier we asserted in \Cref{eq:vacuum-solutions} that we had a vacuum solution. In order for this to be true, our Ricci scalar better be equal to zero, since a vacuum solution actually corresponds to:

\begin{equation}
G_{\mu\nu}=0
\end{equation}

Using \Cref{eq:laplace,eq:R_phiphi=0} and plugging them into \Cref{eq:R} we see that this is indeed the case.
...
Again, generally speaking $G_{zz}=0$, which we can see by plugging \Cref{eq:nu_r} into \Cref{eq:G_zz}. So we have this object which is zero everywhere except along the z-axis.
...
The two-dimensional sub-manifold in $\left(r,\phi\right)$ is diagonal, so that the only non vanishing components of the Ricci tensor are $R_{rr}=R_{\phi\phi}=K$. Applying Einstein's equation (\Cref{eq:einstein}) we have:
...
To address if we are justified in applying \Cref{eq:h-limit}, we should check to see if our solution reduces to the Schwarszchild solution for $r\rightarrow\infty$.
