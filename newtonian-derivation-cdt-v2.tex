...
Applying this complete set of relations to Equation \eqref{eq:ricci-tensor-components} gives the following:
\begin{equation}
...
Equation \eqref{eq:laplace} is the two-dimensional Laplace equation in cylindrical coordinates. Plugging Equation \eqref{eq:R_phiphi=0} into Equation \eqref{eq:R_rr=0} gives:

\begin{equation}
\partial_{r}\nu=r\left(\left(\partial_{r}\lambda\right)^{2}-\left(\partial_{z}\lambda\right)^{2}\right)\label{eq:nu_r}
\end{equation}

Using Equations \eqref{eq:nu_z} and \eqref{eq:nu_r} we find solutions for $\nu$ are given by:
\begin{equation}
...
In order to be certain that our spacetime is truly flat, we want to check for elementary flatness, that is, that the ratio of the circumference and radius is equal to $2\pi$. To do this we will parallel transport a vector $W$ about the $\phi$-axis, find the values for $\phi=0$ and $\phi=2\pi$, and check that their ratio is $2\pi$. This will give us restrictions on our solutions for $\lambda\left(r,z\right)$ and $\nu\left(r,z\right)$.


The relevant Christoffel symbols are $\Gamma^{r}_{\phi\phi}$, $\Gamma^{z}_{\phi\phi}$, $\Gamma^{\phi}_{\phi r}$, and $\Gamma^{\phi}_{\phi z}$, so that our equations are:
...
Differentiating Equation \eqref{eq:W-phi-r} with respect to $\phi$ and plugging it into Equation \eqref{eq:W-r-phi} gives:
...
And from Equation \eqref{eq:W-phi-z}:
