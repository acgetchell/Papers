...
Equation (\ref{eq:nu}) includes a constant, which we can set by choosing that $\nu=0$ at \emph{A}. So, $\nu=0$ along the z-axis from \emph{$\acute{A}$} to \emph{A}. The same applies from \emph{$\acute{B}$} to \emph{B}. Then our path \emph{ADB} may be deformed into an infinite semicircle.

Now consider two bodies, as in the following diagram:

In this case, we expect $\nu=0$ along \emph{$\acute{A}$A} and \emph{$\acute{C}$C}. But there is no $\emph{a priori}$ reason to think that $\nu=0$ along \emph{$\acute{B}$B}.
This means that our vacuum solution fails along the z-axis. Therefore, there must be a strut of matter, i.e. a metric \emph{I} such that $R_{\mu\nu}\neq 0$, along the z-axis \emph{$\acute{B}$B} separating the two objects. This corresponds with the expectation that two masses will attract each other and not remain at rest.

What is the form of \emph{I}?
